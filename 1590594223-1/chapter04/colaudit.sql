...
ALTER SESSION SET tracefile_identifier = 'colaudit';
...
CREATE TABLE gfc_ora_tab_columns 
...
CREATE TABLE gfc_ora_ind_columns 
STORAGE (INITIAL 1M NEXT 1M)
AS
SELECT	table_name
, 	index_name
, 	column_position
, 	substr(column_name,1,100) column_name
...
SELECT	/*COL-04*/
	r.recname, d.table_name, p.pscols, d.dbcols
FROM	psrecdefn r
...
INSERT INTO gfc_rebuild (recname)
SELECT  /*+ORDERED COL-07*/ DISTINCT r.recname
FROM	psrecdefn r
,	gfc_ora_tab_columns d
,	gfc_ps_tab_columns p
,	psdbfield f
...
SELECT	/*COL-10*/
	k.recname, k.indexid, i.table_name, i.INDEX_name, i.dbcols, k.pscols
FROM	(
...
SELECT	/*COL-11*/
	k.recname, k.indexid, k.fieldname, k.keyposn, i.column_position
FROM	psrecdefn r
,	gfc_ps_keydefn k
